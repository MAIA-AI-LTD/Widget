function chatWidget(){let i=this;this.config={},this.init=()=>{this.insertChat(),this.loadMessageHistory(),this.autoSizeWidth(),this.sendingToChat(),this.sendingUsingEnter(),this.textareaCancelNewlineOn(),this.openChat(),this.closeChat(),this.textareaAutoSizeWidth(),this.resizeChatWidth(),this.textareaAutoHeight(),this.checkFirstOpening.init(),this.chatAutoStart.init(),this.iconForUnreadMessage.checkNotifLocaleStorage(),this.insertMetaTag()},this.checkFirstOpening={init(){this.setHandler()},isOpened:!1,setHandler(){i.chatBubble.addEventListener("click",this.handler)},handler(){i.checkFirstOpening.isOpened=!0,this.removeEventListener("click",i.checkFirstOpening.handler)}},this.insertChat=t=>{document.head.insertAdjacentHTML("beforeend",'<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap" rel="stylesheet">');var e=document.createElement("style"),e=(e.innerHTML=`
  ...
  `,document.head.appendChild(e),document.createElement("div"));e.id="chat-widget-container",e.classList.add("chat-widget-container"),document.body.appendChild(e),e.innerHTML=`
  ...
  `,this.elementSearch()},this.elementSearch=()=>{this.chatInput=document.getElementById("chat-input"),this.chatSubmit=document.getElementById("chat-submit"),this.chatMessages=document.getElementById("chat-messages"),this.chatBubble=document.getElementById("chat-bubble"),this.chatPopup=document.getElementById("chat-popup"),this.closePopup=document.getElementById("close-popup"),this.chatHistory=document.getElementById("chat-history")},this.autoSizeWidth=()=>{window.innerWidth<768||this.chatMessages.clientHeight<this.chatMessages.scrollHeight&&(this.chatMessages.style.paddingRight="13px")},this.messagesContainerCheckWidth=()=>{768<window.innerWidth&&this.chatMessages.clientHeight<this.chatMessages.scrollHeight?this.chatMessages.style.paddingRight="13px":this.chatMessages.style.paddingRight="16px"},this.resizeChatWidth=()=>{window.addEventListener("resize",()=>{this.messagesContainerCheckWidth()})},this.textareaCheckWidth=()=>{window.innerWidth<768||(parseFloat(getComputedStyle(this.chatInput).maxHeight)<this.chatInput.scrollHeight?this.chatInput.style.paddingRight="13px":this.chatInput.style.paddingRight="16px")},this.textareaAutoSizeWidth=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckWidth()})},this.textareaCheckHeight=()=>{this.chatInput.style.height="36px",this.chatInput.style.height=this.chatInput.scrollHeight+"px"},this.textareaAutoHeight=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckHeight()})},this.sendingToChat=()=>{this.chatSubmit.addEventListener("click",async()=>{var t=this.chatInput.value.trim();t&&(this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.onUserRequest(t),i.prints.printsAdd(),this.autoSizeWidth(),this.textareaCheckHeight())})},this.sendingUsingEnter=()=>{this.chatInput.addEventListener("keyup",t=>{"Enter"===t.key&&(this.chatSubmit.click(),this.autoSizeWidth())})},this.textareaCancelNewlineOn=()=>{this.chatInput.addEventListener("keydown",t=>{"Enter"===t.key&&t.preventDefault()})},this.openChat=()=>{this.chatBubble.addEventListener("click",()=>{this.togglePopup()})},this.closeChat=()=>{this.closePopup.addEventListener("click",()=>{this.togglePopup()})},this.togglePopup=()=>{this.chatPopup.classList.toggle("hidden")},this.scrollWindowToBottom=()=>{this.chatPopup.classList.remove("hidden"),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.chatPopup.classList.add("hidden")},this.chatAutoStart={init(){this.setTimeoutAutoStart()},setTimeoutAutoStart(){i.config?.seconds_to_autostart&&setTimeout(()=>{i.checkFirstOpening.isOpened||i.chatPopup.classList.contains("hidden")&&i.chatBubble.click()},1e3*i.config?.seconds_to_autostart)}},this.userMessageAppend=t=>{t=this.getUserMessage(t);this.chatMessages.appendChild(t),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.chatInput.value=""},this.onUserRequest=t=>{try{this.userMessageAppend(t),this.ws.send(JSON.stringify({action:"MESSAGE",session_id:localStorage.getItem("session_id"),message:t}))}catch(t){console.log(t)}},this.getBotMessage=t=>{var t=i.convertingFunction.parseLinks(t),e=document.createElement("div");return e.className="flex mb-3 chat-message-bot-container",e.innerHTML=`<div class="chat-message-bot">${t}</div>`,e},this.getUserMessage=t=>{var e=document.createElement("div");return e.className="chat-message-user-container",e.innerHTML=`<div class="chat-message-user">${t}</div>`,e},this.loadMessageHistory=()=>{this.config.conversation_history.forEach(({author:t,message:e})=>{this.chatHistory.append("USER"===t?this.getUserMessage(e):this.getBotMessage(e))}),this.scrollWindowToBottom()},this.reply=t=>{this.messageSound.audioPlay();t=this.getBotMessage(t);this.chatMessages.appendChild(t),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.autoSizeWidth()},this.openWebSoket=(t="apidev.maia.work")=>{var{appId:e,apiHash:i}=document.querySelector("#maia-chat-widget").dataset;this.ws=new WebSocket(`wss://${t}/api/v1/webhook/chat_widget?app_id=${e}&api_hash=`+i),this.webSocketOpenHandler(),this.webSocketMessageHandler(),this.webSocketCloseHandler(),this.websocketClosure()},this.webSocketOpenHandler=()=>{let e=i.helpers.getPhoneNumber();this.ws.onopen=t=>{this.ws.send(JSON.stringify({action:"INIT",session_id:localStorage.getItem("session_id")?localStorage.getItem("session_id"):void 0,metadata:{phone:e||null}}))}},this.webSocketMessageHandler=()=>{this.ws.onmessage=t=>{t=JSON.parse(JSON.parse(t.data));"INIT"===t.action&&(localStorage.getItem("session_id")||localStorage.setItem("session_id",t.session_id),this.writingDataToConfig(t),this.init(),this.messageSound.init(),this.iconForUnreadMessage.init()),"MESSAGE"===t.action&&(this.reply(t.answer),this.iconForUnreadMessage.messageCheck(),i.prints.printsRemove())}},this.webSocketCloseHandler=()=>{this.ws.onclose=t=>{this?.chatInput?.setAttribute("disabled",!0)}},this.websocketClosure=()=>{document.addEventListener("unload",()=>{this.ws.close()})},this.writingDataToConfig=t=>{this.config={...this.config,salesperson_name:t.salesperson_name,salesperson_role:t.salesperson_role,chat_title:t.chat_title,circle_color:t.circle_color,header_color:t.header_color,seconds_to_autostart:t.seconds_to_autostart,send_button_color:t.send_button_color,user_message_color:t.user_message_color,widget_location:t.widget_location,conversation_history:t.conversation_history,allowed_domains:t.allowed_domains}},this.checkDomen=()=>!!window.location.href.includes(this.config.allowed_domains[0])||(console.log("Ваш домен не включен в список доступных доменов"),!1),this.prints={callArray:[],printsLoadingChoiceActionStart(){this.callArray.push(!0),1===this.callArray.length&&this.printsAdd()},printsLoadingChoiceActionFinish(){1===this.callArray.length&&this.printsRemove(),this.callArray.pop()},printsAdd(){var t=document.querySelector(".chat-status");t.querySelector("[data-chat-prints-loading]")||t.prepend(this.getPrintsHTML())},printsRemove(){document.querySelector(".chat-status [data-chat-prints-loading]")?.remove()},getPrintsHTML(){var t=document.createElement("div");return t.innerHTML=`<div class="chat-prints" data-chat-prints-loading>
                                    <div class="chat-prints-circle-container">
                                      <div class="chat-prints-circle chat-prints-circle-first"></div>
                                        ...
                                        печатает
                                    </div>`,t.firstElementChild}},this.messageSound={init(){this.createAudio()},createAudio(){this.sound=document.createElement("audio"),this.sound.src="https://b6905acf-5120-4e0c-b3ea-3716bba15c68.selstorage.ru/message_sound.mp3",this.sound.addEventListener("error",()=>{this.sound=null})},audioPlay(){this.sound&&(this.sound.paused||(this.sound.pause(),this.sound.currentTime=0),this.sound.play())}},this.iconForUnreadMessage={init(){this.chatOpenHandler()},messageCheck(){this.closeСheck()&&(this.showIcon(),this.setNotifLocaleStorage())},"closeСheck"(){var t=document.querySelector("#chat-popup");if(t)return t.classList.contains("hidden")},showIcon(){var t=document.querySelector("#chat-unread-message-icon");t&&t.classList.remove("hidden")},setNotifLocaleStorage(){localStorage.setItem("unred-message",!0)},removeNotifLocaleStorage(){localStorage.removeItem("unred-message")},checkNotifLocaleStorage(){localStorage.getItem("unred-message")&&this.showIcon()},hiddenIcon(){var t=document.querySelector("#chat-unread-message-icon");t&&t.classList.add("hidden")},chatOpenHandler(){var t=document.querySelector("#chat-bubble");t&&t.addEventListener("click",()=>{this.closeСheck()||(this.hiddenIcon(),this.removeNotifLocaleStorage())})}},this.insertMetaTag=()=>{let t=document.querySelector('meta[name="viewport"]');t||((t=document.createElement("meta")).name="viewport",document.getElementsByTagName("head")[0].appendChild(t)),t.content&&(t.content.includes(", user-scalable=no")?t.content="width=device-width, initial-scale=1.0, user-scalable=no":t.content+=", user-scalable=no")},this.convertingFunction={parseLinks(t){var e=/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi;return t.replace(e,function(t){return'<a href="'+t+'" target="_blank">'+t+"</a>"})}},this.helpers={getParams(){var t=window.location.href;return Object.fromEntries(new URL(t).searchParams.entries())},getPhoneNumber(){var t=document.querySelector("#maia-chat-widget");return(t=t&&t.dataset.phone)&&"null"!==t?t:""}}}(new chatWidget).openWebSoket("api.maia.work");