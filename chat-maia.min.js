function chatWidget(){const globalThis=this;this.config={},this.init=()=>{this.insertChat(),this.loadMessageHistory(),this.autoSizeWidth(),this.sendingToChat(),this.sendingUsingEnter(),this.textareaCancelNewlineOn(),this.openChat(),this.closeChat(),this.textareaAutoSizeWidth(),this.resizeChatWidth(),this.textareaAutoHeight(),this.checkFirstOpening.init(),this.chatAutoStart.init()},this.checkFirstOpening={init(){this.setHandler()},isOpened:!1,setHandler(){globalThis.chatBubble.addEventListener("click",this.handler)},handler(){globalThis.checkFirstOpening.isOpened=!0,this.removeEventListener("click",globalThis.checkFirstOpening.handler)}},this.insertChat=colorSettings=>{document.head.insertAdjacentHTML("beforeend",'<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap" rel="stylesheet">');var style=document.createElement("style"),style=(style.innerHTML=`
    ...
    `,document.head.appendChild(style),document.createElement("div"));style.id="chat-widget-container",style.classList.add("chat-widget-container"),document.body.appendChild(style),style.innerHTML=`
    ...
    `,this.elementSearch()},this.elementSearch=()=>{this.chatInput=document.getElementById("chat-input"),this.chatSubmit=document.getElementById("chat-submit"),this.chatMessages=document.getElementById("chat-messages"),this.chatBubble=document.getElementById("chat-bubble"),this.chatPopup=document.getElementById("chat-popup"),this.closePopup=document.getElementById("close-popup"),this.chatHistory=document.getElementById("chat-history")},this.autoSizeWidth=()=>{window.innerWidth<768||this.chatMessages.clientHeight<this.chatMessages.scrollHeight&&(this.chatMessages.style.paddingRight="13px")},this.messagesContainerCheckWidth=()=>{768<window.innerWidth&&this.chatMessages.clientHeight<this.chatMessages.scrollHeight?this.chatMessages.style.paddingRight="13px":this.chatMessages.style.paddingRight="16px"},this.resizeChatWidth=()=>{window.addEventListener("resize",()=>{this.messagesContainerCheckWidth()})},this.textareaCheckWidth=()=>{window.innerWidth<768||(parseFloat(getComputedStyle(this.chatInput).maxHeight)<this.chatInput.scrollHeight?this.chatInput.style.paddingRight="13px":this.chatInput.style.paddingRight="16px")},this.textareaAutoSizeWidth=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckWidth()})},this.textareaCheckHeight=()=>{this.chatInput.style.height="36px",this.chatInput.style.height=this.chatInput.scrollHeight+"px"},this.textareaAutoHeight=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckHeight()})},this.sendingToChat=()=>{this.chatSubmit.addEventListener("click",async()=>{var message=this.chatInput.value.trim();message&&(this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.onUserRequest(message),globalThis.prints.printsAdd(),this.autoSizeWidth(),this.textareaCheckHeight())})},this.sendingUsingEnter=()=>{this.chatInput.addEventListener("keyup",event=>{"Enter"===event.key&&(this.chatSubmit.click(),this.autoSizeWidth())})},this.textareaCancelNewlineOn=()=>{this.chatInput.addEventListener("keydown",e=>{"Enter"===e.key&&e.preventDefault()})},this.openChat=()=>{this.chatBubble.addEventListener("click",()=>{this.togglePopup()})},this.closeChat=()=>{this.closePopup.addEventListener("click",()=>{this.togglePopup()})},this.togglePopup=()=>{this.chatPopup.classList.toggle("hidden")},this.scrollWindowToBottom=()=>{this.chatPopup.classList.remove("hidden"),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.chatPopup.classList.add("hidden")},this.chatAutoStart={init(){this.setTimeoutAutoStart()},setTimeoutAutoStart(){globalThis.config?.seconds_to_autostart&&setTimeout(()=>{globalThis.checkFirstOpening.isOpened||globalThis.chatPopup.classList.contains("hidden")&&globalThis.chatBubble.click()},1e3*globalThis.config?.seconds_to_autostart)}},this.userMessageAppend=message=>{message=this.getUserMessage(message);this.chatMessages.appendChild(message),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.chatInput.value=""},this.onUserRequest=message=>{try{this.userMessageAppend(message),this.ws.send(JSON.stringify({action:"MESSAGE",session_id:localStorage.getItem("session_id"),message:message}))}catch(e){console.log(e)}},this.getBotMessage=message=>{var message=globalThis.convertingFunction.parseLinks(message),bMessage=document.createElement("div");return bMessage.className="flex mb-3 chat-message-bot-container",bMessage.innerHTML=`<div class="chat-message-bot">${message}</div>`,bMessage},this.getUserMessage=message=>{var uMessage=document.createElement("div");return uMessage.className="chat-message-user-container",uMessage.innerHTML=`<div class="chat-message-user">${message}</div>`,uMessage},this.loadMessageHistory=()=>{this.config.conversation_history.forEach(({author,message})=>{this.chatHistory.append("MAIA"===author?this.getBotMessage(message):this.getUserMessage(message))}),this.scrollWindowToBottom()},this.reply=message=>{this.messageSound.audioPlay();message=this.getBotMessage(message);this.chatMessages.appendChild(message),this.chatMessages.scrollTop=this.chatMessages.scrollHeight,this.autoSizeWidth()},this.openWebSoket=(domain="apidev.maia.work")=>{var{appId,apiHash}=document.querySelector("#maia-chat-widget").dataset;this.ws=new WebSocket(`wss://${domain}/api/v1/webhook/chat_widget?app_id=${appId}&api_hash=`+apiHash),this.webSocketOpenHandler(),this.webSocketMessageHandler(),this.webSocketCloseHandler(),this.websocketClosure()},this.webSocketOpenHandler=()=>{this.ws.onopen=e=>{this.ws.send(JSON.stringify({action:"INIT",session_id:localStorage.getItem("session_id")?localStorage.getItem("session_id"):void 0}))}},this.webSocketMessageHandler=()=>{this.ws.onmessage=e=>{e=JSON.parse(JSON.parse(e.data));if("INIT"===e.action){if(localStorage.getItem("session_id")||localStorage.setItem("session_id",e.session_id),this.writingDataToConfig(e),!this.checkDomen())return!1;this.init(),this.messageSound.init()}"MESSAGE"===e.action&&(this.reply(e.answer),globalThis.prints.printsRemove())}},this.webSocketCloseHandler=()=>{this.ws.onclose=e=>{this?.chatInput?.setAttribute("disabled",!0)}},this.websocketClosure=()=>{document.addEventListener("unload",()=>{this.ws.close()})},this.writingDataToConfig=rspns=>{this.config={...this.config,salesperson_name:rspns.salesperson_name,salesperson_role:rspns.salesperson_role,chat_title:rspns.chat_title,circle_color:rspns.circle_color,header_color:rspns.header_color,seconds_to_autostart:rspns.seconds_to_autostart,send_button_color:rspns.send_button_color,user_message_color:rspns.user_message_color,widget_location:rspns.widget_location,conversation_history:rspns.conversation_history,allowed_domains:rspns.allowed_domains}},this.checkDomen=()=>{return!!window.location.href.includes(this.config.allowed_domains[0])||(console.log("Ваш домен не включен в список доступных доменов"),!1)},this.prints={callArray:[],printsLoadingChoiceActionStart(){this.callArray.push(!0),1===this.callArray.length&&this.printsAdd()},printsLoadingChoiceActionFinish(){1===this.callArray.length&&this.printsRemove(),this.callArray.pop()},printsAdd(){var printsContainer=document.querySelector(".chat-status");printsContainer.querySelector("[data-chat-prints-loading]")||printsContainer.prepend(this.getPrintsHTML())},printsRemove(){document.querySelector(".chat-status [data-chat-prints-loading]")?.remove()},getPrintsHTML(){var prints=document.createElement("div");return prints.innerHTML=`<div class="chat-prints" data-chat-prints-loading>
                                      <div class="chat-prints-circle-container">
                                        <div class="chat-prints-circle chat-prints-circle-first"></div>
                                          ...
                                          печатает
                                      </div>`,prints.firstElementChild}},this.messageSound={init(){this.createAudio()},createAudio(){this.sound=document.createElement("audio"),this.sound.src="https://cabinet.maia.work/static/audio/message_sound.mp3",this.sound.addEventListener("error",()=>{this.sound=null})},audioPlay(){this.sound&&(this.sound.paused||(this.sound.pause(),this.sound.currentTime=0),this.sound.play())}},this.convertingFunction={parseLinks(message){var urlRegex=/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi;return message.replace(urlRegex,function(url){return'<a href="'+url+'" target="_blank">'+url+"</a>"})}}}(new chatWidget).openWebSoket("api.maia.work");